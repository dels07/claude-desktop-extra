...
Which chunk holds what also moves between releases - the static registry sat in `index2.chunk-*` in v1.28929.0 and in `index.chunk-*` since v1.30096.1 (registry, merger, GrowthBook store and reader exports all in `index.chunk-RiH_RK8u.js` in v1.34493.1, `index.chunk-CdfeLm1B.js` in v1.32885.1, `index.chunk-BFWQf1ai.js` in v1.32352.1). **An audit concat built from `index.chunk-*` alone is not the bundle**; it will show real, still-present flags as "removed". Always concatenate both families.

## Overview

Feature flags are controlled by a 3-layer system (current minified names — see version-history table for the rename trail):

1. **`uR()` (static, module-scoped)** - Calls individual feature functions, builds base object (was `RH()` in v1.32885.1, `JB()` in v1.32352.1, `$V()` in v1.30096.1, `X()` in v1.26832.0-v1.28929.0, `Ku()` in v1.20186.1-v1.21459.0, `yh()` in v1.19367.0, `sM()` in v1.18286.x). The maturity post-processor is called INLINE (since v1.32885.1) - `function uR(){let e=tZt();return CZt({launch:cR,coworkBrowser:fZt(),artifactsPane:dZt(),nativeQuickEntry:kXt(),...})}` - `launch` is a STATIC always-supported entry (flag `2976814254` is gone from the bundle), not an async override; the yukonSilver/Gems/GemsCache status is hoisted once (`let e=tZt()`, was `V2t()`/`e9t()`). The post-processor `CZt()` (was `l4t()`/`S9t()`/`u$t()`) stamps `maturity:"beta"` onto supported features in a list (beta list const `NYt`, which in v1.34493.1 holds a single entry `[`surfaceTogglesPreview`]`; cosmetic, no gating). Supported constant `cR` (was `LH`/`qB`). 59 static entries in v1.34493.1 (57 in v1.32885.1).
2. **`wZt` (async merger)** (was `u4t` in v1.32885.1, `C9t` in v1.32352.1) - Spreads `uR()` and applies the async overrides: returns `{...uR(),...l}` where `l={louderPenguin,coworkKappa,coworkArtifacts,coworkAutoModeAlwaysAllowOverride,epitaxyMcpApps,coworkWatchRecord,coworkWatchers,ccdGitEngine,sessionPrOwnership,violinBow}` (**10 overrides as of v1.34493.1**, was 9 in v1.32352.1-v1.32885.1; `sessionPrOwnership` still reuses the SAME destructured slot as `ccdGitEngine` (`ccdGitEngine:s,sessionPrOwnership:s`), i.e. both are gated by flag `959099749` via the 5s-delayed probe helper `dR(qL)` (was `zH(SH)`/`YB(jB)`); 8 overrides in v1.26832.0-v1.30096.1, where `deterministicCreatePr` held the last slot until v1.28929.0 replaced it with `ccdGitEngine`). GrowthBook-read overrides: coworkKappa `123929380`, coworkArtifacts `2940196192`, coworkAutoModeAlwaysAllowOverride `4200321681`, epitaxyMcpApps `3516166472`, ccdGitEngine + sessionPrOwnership `959099749`; `coworkWatchers` reads the scheduled-tasks module's `watchersEnabled()` (a config callback, not a direct flag read). **`violinBow` (new in v1.34493.1) is `dR(UYt)` where `function UYt(){return!1}`** - hardcoded off, no flag, so it resolves `{status:"unavailable"}` on every platform (a `ViolinBowDisabledError` class exists for sessions that request it). `markTaskComplete` was removed in v1.17282.0; `launch` left the merger for the static registry (observed v1.26832.0). Two final `Promise.all` slots (`Bk().overlayApplied()` and `lR()`) are awaited but not destructured into the override object.
3. **IPC handler** - Calls merger, validates against schema, sends to renderer

**Reader identity is greppable, don't guess it.** The GrowthBook chunk (`index.chunk-RiH_RK8u.js` in v1.34493.1, `index.chunk-CdfeLm1B.js` in v1.32885.1, `index.chunk-BFWQf1ai.js` in v1.32352.1, `index.chunk-BzNP_oYx.js` in v1.30096.1) ends its module with an export map that pairs the readable API name to that release's minified symbol:

```javascript
// v1.34493.1
e.r({areFeaturesLoaded:()=>BS,getFeatureValue:()=>YS,getParsedFeatureValueForKey:()=>eC,
     growthbookEmitter:()=>wS,initGrowthBook:()=>Uit,isFeatureEnabled:()=>XS,
     isFeatureEnabledWithDefault:()=>Yit,isGrowthBookFreshForCurrentAccount:()=>ZS,
     isGrowthBookFreshWithin:()=>QS,observeFeatureGate:()=>GS,onFeatureChange:()=>WS,
     onGrowthBookFreshChange:()=>$S,refreshGrowthBook:()=>US,
     waitForGrowthBookReady:()=>VS,waitForGrowthBookSettled:()=>HS})
```

Grep `isFeatureEnabled:\(\)=>` first; everything else follows from that block. New exports in v1.32352.1: **`isFeatureEnabledWithDefault`** (`$ct(id,dflt)` - boolean read with an explicit fallback, e.g. `$ct(\`3018088575\`,!0)`), `isGrowthBookFreshForCurrentAccount`, `onGrowthBookFreshChange`. **Reader shapes** — bare in the GrowthBook chunk, module-qualified elsewhere (the import alias varies per chunk):

| Role | Bare (v1.34493.1) | Dotted (v1.34493.1) | v1.32885.1 (bare/dotted) | v1.32352.1 (bare/dotted) | v1.30096.1 (bare/dotted) |
|------|-------------------|---------------------|--------------------------|--------------------------|--------------------------|
| boolean | `XS(id)` | `.Sb(id)` | `Iw` / `.Ey` | `KE` / `.ty` | `AI` / `.Qu` |
| boolean with default | `Yit(id,dflt)` | `.Cb(id,dflt)` | `Uit` / - | `$ct` / - | - (new export) |
| value with default | `YS(id,dflt)` | `.yb(id,dflt)` | `Fw` / `.Cy` | `GE` / `.Qv` | `kI` / `.Yu` |
| keyed config value | `eC(id,key,dflt,zodType)` | `.bb(id,key,dflt,zodType)` | `zw` / `.wy` | `XE` / `.$v` | `NI` / `.Xu` |
| change listener | `WS(id,cb)` | `.Eb(id,cb)` | `Aw` / `.ky` | `RE` / `.iy` | `wI` / `.td` |

(In v1.34493.1: `observeFeatureGate` = bare `GS` / dotted `.Tb`, `areFeaturesLoaded` = `BS` / `.vb`, `isGrowthBookFreshForCurrentAccount` = `ZS` / `.wb`, `waitForGrowthBookSettled` = `HS` / `.kb`, `onGrowthBookFreshChange` = `$S` / `.Db`, `refreshGrowthBook` = `US` / `.Ob`. The dotted names are `Object.defineProperty(exports,"<name>",...)` re-exports on the same chunk - grep `exports,"XX",{enumerable:!0,get:function(){return <bare>}` to rebuild the map. Older columns live in the version-history table.)
...
## The Production Gate `sR()` (was `IH()` in v1.32885.1, `KB()` in v1.32352.1, `YV()` in v1.30096.1, `LM()` through v1.28929.0, `gM()` in v1.15962.x, `HR()` in v1.15200.0, historically `PM()`/`Nb()`/`DT()`/`MW()`)

In v1.34493.1: ``function sR(e){return o.app.isPackaged?{status:`unavailable`}:e()}`` (electron var `o`, unchanged shape).
...
### Layer 1: Static Registry (`uR()` in v1.34493.1; the sketch below uses the older `Np()`/`PM()` names)
...
### Layer 2: Async Merger (`wZt` in v1.34493.1; was `u4t` in v1.32885.1, `C9t` in v1.32352.1, `nH` in v1.30096.1, `We` in v1.26832.0-v1.28929.0, `Yue`, `HSA` in v1.15962.x, `UcA` in v1.8089.1, `woA` in v1.7196.0)
