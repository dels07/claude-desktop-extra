...
| [`fix_startup_settings.nim`](patches/linux/fix_startup_settings.nim) | Hides the main window when the app is launched by session restore, keeps "Start at login" entries separate per profile, and points the autostart entry at the launcher so a login launch gets the same Wayland, PATH and profile setup as a manual one |
