...
Which chunk holds what also moves between releases - the static registry sat in `index2.chunk-*` in v1.28929.0 and in `index.chunk-*` since v1.30096.1 (registry, merger, GrowthBook store and reader exports all in `index.chunk-CdfeLm1B.js` in v1.32885.1, `index.chunk-BFWQf1ai.js` in v1.32352.1). **An audit concat built from `index.chunk-*` alone is not the bundle**; it will show real, still-present flags as "removed". Always concatenate both families.

## Overview

Feature flags are controlled by a 3-layer system (current minified names — see version-history table for the rename trail):

1. **`RH()` (static, module-scoped)** - Calls individual feature functions, builds base object (was `JB()` in v1.32352.1, `$V()` in v1.30096.1, `X()` in v1.26832.0-v1.28929.0, `Ku()` in v1.20186.1-v1.21459.0, `yh()` in v1.19367.0, `sM()` in v1.18286.x). As of v1.32885.1 the maturity post-processor is called INLINE - `function RH(){let e=V2t();return l4t({launch:LH,coworkBrowser:Q2t(),artifactsPane:Z2t(),nativeQuickEntry:C2t(),...})}` - `launch` is a STATIC always-supported entry (flag `2976814254` is gone from the bundle), not an async override; the yukonSilver/Gems/GemsCache status is hoisted once (`let e=V2t()`, was `e9t()`). The post-processor `l4t()` (was `S9t()`/`u$t()`) stamps `maturity:"beta"` onto supported features in a list (beta list const `P0t`; cosmetic, no gating).
2. **`u4t` (async merger)** (was `C9t` in v1.32352.1) - Spreads `RH()` and applies the async overrides: returns `{...RH(),...c}` where `c={louderPenguin,coworkKappa,coworkArtifacts,coworkAutoModeAlwaysAllowOverride,epitaxyMcpApps,coworkWatchRecord,coworkWatchers,ccdGitEngine,sessionPrOwnership}` (**9 overrides as of v1.32352.1, unchanged in v1.32885.1** - `sessionPrOwnership` reuses the SAME destructured slot as `ccdGitEngine` (`ccdGitEngine:s,sessionPrOwnership:s`), i.e. both are gated by flag `959099749` via the 5s-delayed probe helper `zH(SH)` (was `YB(jB)`); 8 overrides in v1.26832.0-v1.30096.1, where `deterministicCreatePr` held the last slot until v1.28929.0 replaced it with `ccdGitEngine`). GrowthBook-read overrides: coworkKappa `123929380`, coworkArtifacts `2940196192`, coworkAutoModeAlwaysAllowOverride `4200321681`, epitaxyMcpApps `3516166472`, ccdGitEngine + sessionPrOwnership `959099749`; `coworkWatchers` reads the scheduled-tasks module's `watchersEnabled()` (a config callback, not a direct flag read - the former `2678393595` watcher gate was removed). `markTaskComplete` was removed in v1.17282.0; `launch` left the merger for the static registry (observed v1.26832.0). A final `Promise.all` slot (`kN().overlayApplied()`) is awaited but not destructured into the override object.
3. **IPC handler** - Calls merger, validates against schema, sends to renderer

**Reader identity is greppable, don't guess it.** The GrowthBook chunk (`index.chunk-CdfeLm1B.js` in v1.32885.1, `index.chunk-BFWQf1ai.js` in v1.32352.1, `index.chunk-BzNP_oYx.js` in v1.30096.1) ends its module with an export map that pairs the readable API name to that release's minified symbol:

```javascript
// v1.32885.1
e.r({areFeaturesLoaded:()=>Ew,getFeatureValue:()=>Fw,getParsedFeatureValueForKey:()=>zw,
     growthbookEmitter:()=>pw,initGrowthBook:()=>Lit,isFeatureEnabled:()=>Iw,
     isFeatureEnabledWithDefault:()=>Uit,isGrowthBookFreshForCurrentAccount:()=>Lw,
     isGrowthBookFreshWithin:()=>Wit,observeFeatureGate:()=>jw,onFeatureChange:()=>Aw,
     onGrowthBookFreshChange:()=>Rw,refreshGrowthBook:()=>kw,
     waitForGrowthBookReady:()=>Dw,waitForGrowthBookSettled:()=>Ow})
```
...
| Role | Bare (v1.32885.1) | Dotted (v1.32885.1) | v1.32352.1 (bare/dotted) | v1.30096.1 (bare/dotted) | v1.28929.0 |
|------|-------------------|---------------------|--------------------------|--------------------------|------------|
| boolean | `Iw(id)` | `.Ey(id)` | `KE` / `.ty` | `AI` / `.Qu` | `.Yt` |
| boolean with default | `Uit(id,dflt)` | - | `$ct` / - | - (new export) | - |
| value with default | `Fw(id,dflt)` | `.Cy(id,dflt)` | `GE` / `.Qv` | `kI` / `.Yu` | `.Gt` |
| keyed config value | `zw(id,key,dflt,zodType)` | `.wy(id,key,dflt,zodType)` | `XE` / `.$v` | `NI` / `.Xu` | `.Kt` |
| change listener | `Aw(id,cb)` | `.ky(id,cb)` | `RE` / `.iy` | `wI` / `.td` | `.Qt` / `.$t` |

(`observeFeatureGate` dotted is `.Oy` in v1.32885.1. Older columns live in the version-history table.)
...
## The Production Gate `YV()` (was `LM()` through v1.28929.0, `gM()` in v1.15962.x, `HR()` in v1.15200.0, historically `PM()`/`Nb()`/`DT()`/`MW()`)
...
### Layer 1: Static Registry (`$V()` in v1.30096.1; the sketch below uses the older `Np()`/`PM()` names)
...
### Layer 2: Async Merger (`nH` in v1.30096.1; was `We` in v1.26832.0-v1.28929.0, `Yue`, `HSA` in v1.15962.x, `UcA` in v1.8089.1, `woA` in v1.7196.0)
