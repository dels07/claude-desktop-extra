...
The official `.deb` (apt repo `https://downloads.claude.ai/claude-desktop/apt`) bundles its own **Electron** (42.9.2 as of v1.32885.1; authoritative source: the deb tree's `usr/lib/claude-desktop/version` file) and a **native Cowork VM backend**. We download it, verify GPG + SHA256, `dpkg-deb -x` it, patch its `app.asar`, and repackage. We no longer ingest the Windows MSIX, and we no longer pin Electron or rebuild node-pty (both are bundled in the official `.deb`).
...
patches/linux/     #   Linux compatibility - always on, not user-configurable (30)
