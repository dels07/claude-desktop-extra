...
Which chunk holds what also moves between releases - the static registry sat in `index2.chunk-*` in v1.28929.0 and in `index.chunk-*` in v1.30096.1. **An audit concat built from `index.chunk-*` alone is not the bundle**; it will show real, still-present flags as "removed". Always concatenate both families.

## Overview

Feature flags are controlled by a 3-layer system (current minified names — see version-history table for the rename trail):

1. **`$V()` (static, module-scoped)** - Calls individual feature functions, builds base object (was `X()` in v1.26832.0-v1.28929.0, `Ku()` in v1.20186.1-v1.21459.0, `yh()` in v1.19367.0, `sM()` in v1.18286.x). As of v1.30096.1 the registry leads with `launch:XV,coworkBrowser:e$t(),artifactsPane:QV(),nativeQuickEntry:jQt(),...` - `launch` is a STATIC always-supported entry (flag `2976814254` is gone from the bundle), not an async override. The result is post-processed by `u$t()` (was `Ue()`) which stamps `maturity:"beta"` onto supported features in a list (cosmetic, no gating).
2. **`nH` (async merger)** - Spreads `$V()` and applies the async overrides: returns `{...$V(),...c}` where `c={louderPenguin,coworkKappa,coworkArtifacts,coworkAutoModeAlwaysAllowOverride,epitaxyMcpApps,coworkWatchRecord,coworkWatchers,ccdGitEngine}` (**8 overrides, unchanged in v1.28929.0 and v1.30096.1**; already 8 in v1.26832.0 with `deterministicCreatePr` in the last slot - that feature was removed in v1.28929.0 and the slot became `ccdGitEngine`, gated by flag `959099749` via the 5s-delayed probe helper, `tH()` in v1.30096.1). GrowthBook-read overrides: coworkKappa `123929380`, coworkArtifacts `2940196192`, coworkAutoModeAlwaysAllowOverride `4200321681`, epitaxyMcpApps `3516166472`, ccdGitEngine `959099749`; `coworkWatchers` reads the scheduled-tasks module's `watchersEnabled()` (a config callback, not a direct flag read - the former `2678393595` watcher gate was removed). `markTaskComplete` was removed in v1.17282.0; `launch` left the merger for the static registry (observed v1.26832.0). A 9th `Promise.all` slot (`rI().overlayApplied()`) is awaited but not destructured into the override object.
3. **IPC handler** - Calls merger, validates against schema, sends to renderer

**Reader identity is greppable, don't guess it.** The GrowthBook chunk (`index.chunk-BzNP_oYx.js` in v1.30096.1) ends its module with an export map that pairs the readable API name to that release's minified symbol:

```javascript
e.r({areFeaturesLoaded:()=>$Mt,getFeatureValue:()=>kI,getParsedFeatureValueForKey:()=>NI,
     growthbookEmitter:()=>cI,initGrowthBook:()=>ZMt,isFeatureEnabled:()=>AI,
     observeFeatureGate:()=>TI,onFeatureChange:()=>wI,refreshGrowthBook:()=>CI,
     waitForGrowthBookReady:()=>xI,waitForGrowthBookSettled:()=>SI})
```

Grep `isFeatureEnabled:\(\)=>` first; everything else follows from that block. **v1.30096.1 reader shapes** — bare in the GrowthBook chunk, module-qualified elsewhere (the import alias varies per chunk):

| Role | Bare | Dotted (v1.30096.1) | v1.28929.0 | v1.26832.0 |
|------|------|---------------------|------------|------------|
| boolean | `AI(id)` | `.Qu(id)` | `.Yt` | `.Zt` |
| value with default | `kI(id,dflt)` | `.Yu(id,dflt)` | `.Gt` | `.qt` |
| keyed config value | `NI(id,key,dflt,zodType)` | `.Xu(id,key,dflt,zodType)` | `.Kt` | - |
| change listener | `wI(id,cb)` | `.td(id,cb)` | `.Qt` / `.$t` | - |
