...
Which chunk holds what also moves between releases - the static registry sat in `index2.chunk-*` in v1.28929.0 and in `index.chunk-*` in v1.30096.1 and v1.32352.1 (registry, merger, GrowthBook store and reader exports all in `index.chunk-BFWQf1ai.js` in v1.32352.1). **An audit concat built from `index.chunk-*` alone is not the bundle**; it will show real, still-present flags as "removed". Always concatenate both families.

## Overview

Feature flags are controlled by a 3-layer system (current minified names — see version-history table for the rename trail):

1. **`JB()` (static, module-scoped)** - Calls individual feature functions, builds base object (was `$V()` in v1.30096.1, `X()` in v1.26832.0-v1.28929.0, `Ku()` in v1.20186.1-v1.21459.0, `yh()` in v1.19367.0, `sM()` in v1.18286.x). As of v1.32352.1 the registry leads with `launch:qB,coworkBrowser:d9t(),artifactsPane:u9t(),nativeQuickEntry:I7t(),...` - `launch` is a STATIC always-supported entry (flag `2976814254` is gone from the bundle), not an async override; the yukonSilver/Gems/GemsCache status is hoisted once (`let e=e9t()`). The result is post-processed by `S9t()` (was `u$t()`) which stamps `maturity:"beta"` onto supported features in a list (cosmetic, no gating).
2. **`C9t` (async merger)** - Spreads `JB()` and applies the async overrides: returns `{...JB(),...c}` where `c={louderPenguin,coworkKappa,coworkArtifacts,coworkAutoModeAlwaysAllowOverride,epitaxyMcpApps,coworkWatchRecord,coworkWatchers,ccdGitEngine,sessionPrOwnership}` (**9 overrides as of v1.32352.1** - new `sessionPrOwnership` reuses the SAME destructured slot as `ccdGitEngine` (`ccdGitEngine:s,sessionPrOwnership:s`), i.e. both are gated by flag `959099749` via the 5s-delayed probe helper `YB(jB)`; 8 overrides in v1.26832.0-v1.30096.1, where `deterministicCreatePr` held the last slot until v1.28929.0 replaced it with `ccdGitEngine`). GrowthBook-read overrides: coworkKappa `123929380`, coworkArtifacts `2940196192`, coworkAutoModeAlwaysAllowOverride `4200321681`, epitaxyMcpApps `3516166472`, ccdGitEngine + sessionPrOwnership `959099749`; `coworkWatchers` reads the scheduled-tasks module's `watchersEnabled()` (a config callback, not a direct flag read - the former `2678393595` watcher gate was removed). `markTaskComplete` was removed in v1.17282.0; `launch` left the merger for the static registry (observed v1.26832.0). A final `Promise.all` slot (`kN().overlayApplied()`) is awaited but not destructured into the override object.
3. **IPC handler** - Calls merger, validates against schema, sends to renderer

**Reader identity is greppable, don't guess it.** The GrowthBook chunk (`index.chunk-BFWQf1ai.js` in v1.32352.1, `index.chunk-BzNP_oYx.js` in v1.30096.1) ends its module with an export map that pairs the readable API name to that release's minified symbol:

```javascript
// v1.32352.1
e.r({areFeaturesLoaded:()=>PE,getFeatureValue:()=>GE,getParsedFeatureValueForKey:()=>XE,
     growthbookEmitter:()=>_E,initGrowthBook:()=>ME,isFeatureEnabled:()=>KE,
     isFeatureEnabledWithDefault:()=>$ct,isGrowthBookFreshForCurrentAccount:()=>qE,
     isGrowthBookFreshWithin:()=>JE,observeFeatureGate:()=>zE,onFeatureChange:()=>RE,
     onGrowthBookFreshChange:()=>YE,refreshGrowthBook:()=>LE,
     waitForGrowthBookReady:()=>FE,waitForGrowthBookSettled:()=>IE})
```
...
| Role | Bare (v1.32352.1) | Dotted (v1.32352.1) | v1.30096.1 (bare/dotted) | v1.28929.0 | v1.26832.0 |
|------|-------------------|---------------------|--------------------------|------------|------------|
| boolean | `KE(id)` | `.ty(id)` | `AI` / `.Qu` | `.Yt` | `.Zt` |
| boolean with default | `$ct(id,dflt)` | - | - (new export) | - | - |
| value with default | `GE(id,dflt)` | `.Qv(id,dflt)` | `kI` / `.Yu` | `.Gt` | `.qt` |
| keyed config value | `XE(id,key,dflt,zodType)` | `.$v(id,key,dflt,zodType)` | `NI` / `.Xu` | `.Kt` | - |
| change listener | `RE(id,cb)` | `.iy(id,cb)` | `wI` / `.td` | `.Qt` / `.$t` | - |
